...
def table_inserter(df):
    ...
    if len(df.columns) > len(df.columns.drop_duplicates()):
        ...
        if first_cond and duplicates:
            #slice based on start index
            identify_lst_slicing = [idx for idx, col in enumerate(df.columns) if col == df.columns[0]]
            
            #['maturity', 'amount', 'interest_rate', 'yield', 'cusip', 'nan','amount', 'interest_rate', 'yield', 'cusip']
            # maturity is came as nan in second indexing so look for end field index
...
def callable_cusip_extractor(basename,page_num= 0,footer_pattern=None):
    ...
    with open(f'/home/dhivakar/pdf_table_extractor/{id}.csv', 'w+') as f:
    ...
    for page_no, dfs in df_map.items():
        for df in dfs:
            ...
            with open(f'/home/dhivakar/pdf_table_extractor/{id}.csv', 'a') as f:
                print(f'\n', file=f)
                # print(df)
